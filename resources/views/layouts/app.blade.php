    ...
    <aside class="fixed inset-y-0 left-0 z-50 w-64 bg-gradient-to-b from-blue-900 to-blue-800 shadow-2xl transform transition-transform duration-300 ease-in-out lg:relative lg:translate-x-0"
        ...
        <div class="flex items-center justify-between h-16 px-6 border-b border-blue-700">
            <div class="flex items-center space-x-3">
                ...
                <div>
                    <p class="text-white font-bold text-sm leading-tight">Sistem Pakar</p>
                    <p class="text-blue-300 text-xs">Prestasi Siswa</p>
                </div>
            </div>
...
<nav class="px-4 py-4 space-y-1 overflow-y-auto h-full pb-20">
    ...
    <a href="{{ route('penilaian.index') }}"
       class="flex items-center space-x-3 px-3 py-2.5 rounded-lg transition duration-200
       {{ request()->routeIs('penilaian.index') || request()->routeIs('penilaian.create') || request()->routeIs('penilaian.edit') || request()->routeIs('penilaian.show') || request()->routeIs('penilaian.store') ? 'bg-white/20 text-white' : 'text-blue-200 hover:bg-white/10 hover:text-white' }}">
        <svg class="w-5 h-5 flex-shrink-0" fill="none" stroke="currentColor" viewBox="0 0 24 24">
    ...
    {{-- Ranking --}}
    <a href="{{ route('penilaian.ranking') }}"
       class="flex items-center space-x-3 px-3 py-2.5 rounded-lg transition duration-200
       {{ request()->routeIs('penilaian.ranking') ? 'bg-white/20 text-white' : 'text-blue-200 hover:bg-white/10 hover:text-white' }}">
        <svg class="w-5 h-5 flex-shrink-0" fill="none" stroke="currentColor" viewBox="0 0 24 24">
            <path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M11.049 2.927c.3-.921 1.603-.921 1.902 0l1.519 4.674a1 1 0 00.95.69h4.915c.969 0 1.371 1.24.588 1.81l-3.976 2.888a1 1 0 00-.363 1.118l1.518 4.674c.3.922-.755 1.688-1.538 1.118l-3.976-2.888a1 1 0 00-1.176 0l-3.976 2.888c-.783.57-1.838-.197-1.538-1.118l1.518-4.674a1 1 0 00-.363-1.118l-3.976-2.888c-.784-.57-.38-1.81.588-1.81h4.914a1 1 0 00.951-.69l1.519-4.674z"/>
        </svg>
        <span class="text-sm font-medium">Ranking Siswa</span>
    </a>
    ...
    {{-- Kelola Guru (hanya Admin, tanpa label) --}}
    @if(auth()->user()->role === 'admin')
