...
@section('title', 'Ranking Per Kelas')

@section('content')

<div class="mb-6">
    <h2 class="text-xl font-bold text-gray-800 dark:text-white">Ranking Per Kelas</h2>
    <p class="text-sm text-gray-500 dark:text-gray-400">
        @if(auth()->user()->isGuru())
        Ranking siswa kelas {{ auth()->user()->kelas }}
        @else
...
<div class="bg-white dark:bg-gray-800 rounded-xl shadow-lg p-4 mb-6">
    <form action="{{ route('ranking.kelas') }}" method="GET">
        <div class="flex flex-col sm:flex-row gap-3">
            ...
            @if($semester || $kelas)
            <a href="{{ route('ranking.kelas') }}" class="btn-secondary px-4">Reset</a>
            @endif
        </div>
    </form>
</div>
    ...
    <div class="overflow-x-auto">
        <table class="w-full text-sm">
            <thead class="bg-gray-50 dark:bg-gray-700">
                <tr>
                    <th class="px-6 py-3 text-left text-xs font-semibold text-gray-500 uppercase tracking-wider">Peringkat</th>
                    ...
                    <th class="px-6 py-3 text-left text-xs font-semibold text-gray-500 uppercase tracking-wider">Skor</th>
            ...
            <tbody class="divide-y divide-gray-100 dark:divide-gray-700">
                ...
                <tr class="hover:bg-gray-50 dark:hover:bg-gray-700 transition duration-150
                    {{ $item->rank === 1 ? 'bg-yellow-50 dark:bg-yellow-900/20' : '' }}">
                    <td class="px-6 py-4">
                        @if($item->rank === 1)
                    ...
                    <td class="px-6 py-4 text-gray-600 dark:text-gray-300">{{ $item->nilai }}</td>
